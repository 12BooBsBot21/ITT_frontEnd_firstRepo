...
function addContact(name, job, number) {
    if (!detectName(name) || !detectJob(job) || !detectNumber(number))
        return;
...
function groupContactsByLetter() {
    const letters = {};
    for (const l of lettersStr)
        letters[l] = [];
    state.contacts.forEach(c => {
        if (!c.name || typeof c.name !== "string")
            return; // проверка для избегания багов + early return 
        let firstLetter = c.name[0].toLowerCase();
        if (letters[firstLetter])
            letters[firstLetter].push(c);
    });
    return letters;
}
;
...
function renderContacstList(contactsArr, container) {
    container.innerHTML = "";
